...
int main(int argc, char **argv) {
  ...
  while (true) {
    ...
    if (client_fd >= 0) {
      ...
      // Create seperate thread for each Connection
      connections.push_back(std::thread(handleConnection, client_fd));
      // Detach thread, so it can operate independantly
      // new_thread.detach();  // => Doesn't work without -pthread compile flag
    }
  }
